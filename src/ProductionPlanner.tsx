...
export default function ProductionPlanner(){
  ...
  const onHoldSection=<section className="on-hold-panel"><div className="on-hold-heading">ON HOLD <span>{onHoldJobs.length}</span></div>{onHoldJobs.length===0?<p>No jobs on hold.</p>:onHoldJobs.map(job=><article className="on-hold-card" key={job.id} onDoubleClick={()=>openEditor(job)}><strong>{job.title||"Untitled job"}</strong><span>{job.onHoldReason||"No reason provided"}</span></article>)}</section>;

  return <main className="app-shell">{showBacklog&&onHoldSection}
