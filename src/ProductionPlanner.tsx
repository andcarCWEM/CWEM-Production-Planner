...
export default function ProductionPlanner(){
  ...
  const sortedJobs=useMemo(()=>{const categoryLabel=(id:Category)=>categories.find(c=>c.id===id)?.label||id;const operatorName=(id:string|null)=>operators.find(o=>o.id===id)?.name||"Unscheduled";const listJobs=jobs.filter(job=>sortMode==="unavailability"?job.category==="unavailable":job.category!=="unavailable"&&(sortMode==="allocated"?Boolean(job.operatorId):sortMode==="unallocated"?!job.operatorId:true));return [...listJobs].sort((a,b)=>{if(sortMode==="manual")return jobs.indexOf(a)-jobs.indexOf(b);if(sortMode==="revenue")return categoryLabel(a.category).localeCompare(categoryLabel(b.category));if(sortMode==="priority")return (a.priority||Infinity)-(b.priority||Infinity);if(sortMode==="machine")return (a.machine||"ZZZ").localeCompare(b.machine||"ZZZ");if(sortMode==="operator")return operatorName(a.operatorId).localeCompare(operatorName(b.operatorId));if(sortMode==="unavailability")return operatorName(a.operatorId).localeCompare(operatorName(b.operatorId));if(sortMode==="allocated")return operatorName(a.operatorId).localeCompare(operatorName(b.operatorId));if(sortMode==="unallocated")return categoryLabel(a.category).localeCompare(categoryLabel(b.category));return a.duration-b.duration;});},[jobs,operators,sortMode]);
