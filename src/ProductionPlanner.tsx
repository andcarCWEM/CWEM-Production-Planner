...
export default function ProductionPlanner(){
  ...
  const segmentFor=(job:Job)=>segmentForWeek(job,currentWeekKey);
  ...
  const metrics=useMemo(()=>{ const visible=viewMode==="month"?monthJobs:scheduled,planned=visible.reduce((n,j)=>{if(viewMode==="week")return n+(segmentFor(j)?.duration||0);const start=Math.max(jobGlobalStart(j),monthGlobalStart),end=Math.min(jobGlobalStart(j)+j.duration,monthGlobalStart+monthTotalMinutes);return n+Math.max(0,end-start);},0)/60,capacity=operators.length*(TOTAL_MINUTES/60)*(viewMode==="month"?monthWeeks:1); return {planned,utilisation:capacity?Math.round(planned/capacity*100):0,urgent:visible.filter(j=>j.category==="urgent").length}; },[scheduled,operators,jobs,currentWeekGlobal,viewMode,monthGlobalStart,monthTotalMinutes,monthWeeks]);
  ...
  const openGridMenu=(e:React.MouseEvent,board:HTMLDivElement|null,total:number,globalStart:number,rowHeight:number,laneHeight:number)=>{e.preventDefault();if(!board||!operators.length)return;const rect=board.getBoundingClientRect(),x=Math.max(0,Math.min(rect.width,e.clientX-rect.left)),y=Math.max(0,Math.min(rect.height-1,e.clientY-rect.top)),track=Math.max(0,Math.min(operators.length*3-1,Math.floor(y/laneHeight)));setContextMenu({x:Math.min(e.clientX,window.innerWidth-190),y:Math.min(e.clientY,window.innerHeight-90),target:{global:globalStart+Math.round((x/rect.width*total)/15)*15,operatorId:operators[Math.floor(track/3)].id,lane:track%3}});};
  ...
  return <main className="app-shell">
    ...
    <section className="summary-strip"><div><span>{viewMode==="week"?"Week commencing":"Planning month"}</span><strong>{viewMode==="week"?prettyDate(weekStart):monthLabel}</strong></div><div><span>Planned hours</span><strong>{metrics.planned.toFixed(1)} h</strong></div><div><span>Indicative loading</span><strong>{metrics.utilisation}%</strong></div><div><span>Unscheduled</span><strong>{backlog.length}</strong></div><div className={metrics.urgent?"urgent-metric":""}><span>Urgent jobs</span><strong>{metrics.urgent}</strong></div></section>
    <section className="planner-card"><div className="planner-toolbar"><div className="week-nav"><button aria-label={viewMode==="week"?"Previous week":"Previous month"} onClick={()=>viewMode==="week"?setWeekStart(addDays(weekStart,-7)):shiftMonth(-1)}>‹</button><button className="today" onClick={()=>{const now=new Date();if(viewMode==="week")setWeekStart(mondayOf(now));else setMonthCursor(new Date(now.getFullYear(),now.getMonth(),1));}}>Today</button><button aria-label={viewMode==="week"?"Next week":"Next month"} onClick={()=>viewMode==="week"?setWeekStart(addDays(weekStart,7)):shiftMonth(1)}>›</button><h2>{viewMode==="week"?weekLabel:monthLabel}</h2></div>{viewMode==="week"&&<div className="four-week-nav" aria-label="Four week planning horizon">{Array.from({length:4},(_,i)=>{const d=addDays(mondayOf(new Date()),i*7);return <button className={dateKey(d)===currentWeekKey?"active":""} key={i} onClick={()=>setWeekStart(d)}>W{i+1}<span>{prettyDate(d)}</span></button>;})}</div>}<div className="toolbar-right"><div className="view-switch" aria-label="Planner view"><button className={viewMode==="week"?"active":""} onClick={()=>setViewMode("week")}>Week</button><button className={viewMode==="month"?"active":""} onClick={()=>{setMonthCursor(new Date(weekStart.getFullYear(),weekStart.getMonth(),1));setViewMode("month");}}>Month</button></div><span className="autosave"><i/> Saved on this device</span>{viewMode==="week"&&<button className={`backlog-toggle ${showBacklog?"active":""}`} onClick={()=>setShowBacklog(!showBacklog)}>Unscheduled ({backlog.length})</button>}</div></div>
      {viewMode==="week"&&<div className={`planner-body ${showBacklog?"with-backlog":""}`}><div className="schedule-wrap">
      ...
      {viewMode==="month"&&<div className="month-view">
        ...
        <p className="month-help">Drag jobs across dates, operators and lanes · weekends are excluded · double-click to edit</p>
      </div>}
    </section>
