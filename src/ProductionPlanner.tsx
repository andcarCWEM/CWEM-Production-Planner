...
type Job = {
  ...
  onHold: boolean; onHoldReason: string;
};
...
export default function ProductionPlanner(){
  ...
  const [monthCursor,setMonthCursor]=useState(()=>mondayOf(new Date()));
  ...
  useEffect(()=>{
    const updateDisplayHeight=()=>setDisplayHeight(window.screen.availHeight||window.screen.height);
    updateDisplayHeight();
    window.addEventListener("resize",updateDisplayHeight);
    return()=>window.removeEventListener("resize",updateDisplayHeight);
  },[]);
  ...
  const scheduled=jobs.filter(j=>!j.onHold&&j.operatorId&&segmentFor(j)),backlog=jobs.filter(j=>!j.operatorId),onHoldJobs=jobs.filter(j=>j.onHold);
  const sortedJobs=useMemo(()=>{const categoryLabel=(id:Category)=>categories.find(c=>c.id===id)?.label||id;const operatorName=(id:string|null)=>operators.find(o=>o.id===id)?.name||"Unscheduled";const worksOrderKey=(job:Job)=>{const match=job.title.match(/\d+/);return match?Number(match[0]):Infinity;};const listJobs=jobs.filter(job=>sortMode==="onhold"?job.onHold:sortMode==="unallocated"?!job.operatorId&&job.category!=="unavailable":!job.onHold&&(sortMode==="unavailability"?job.category==="unavailable":job.category!=="unavailable"&&(sortMode==="allocated"?Boolean(job.operatorId):true)));return [...listJobs].sort((a,b)=>{if(sortMode==="manual")return jobs.indexOf(a)-jobs.indexOf(b);if(sortMode==="revenue")return categoryLabel(a.category).localeCompare(categoryLabel(b.category));if(sortMode==="priority")return (a.priority||Infinity)-(b.priority||Infinity);if(sortMode==="machine")return (a.machine||"ZZZ").localeCompare(b.machine||"ZZZ");if(sortMode==="operator")return operatorName(a.operatorId).localeCompare(operatorName(b.operatorId));if(sortMode==="unavailability")return operatorName(a.operatorId).localeCompare(operatorName(b.operatorId));if(sortMode==="allocated")return operatorName(a.operatorId).localeCompare(operatorName(b.operatorId));if(sortMode==="unallocated"){const revenueOrder=categoryLabel(a.category).localeCompare(categoryLabel(b.category));return revenueOrder||worksOrderKey(a)-worksOrderKey(b)||a.title.localeCompare(b.title);}return a.duration-b.duration;});},[jobs,operators,sortMode]);
  useEffect(()=>{document.querySelectorAll<HTMLElement>(".backlog-list .backlog-job").forEach((card,index)=>{const job=sortedJobs[index];if(job?.onHold){card.classList.add("on-hold-job");if(!card.querySelector(".on-hold-indicator")){const marker=document.createElement("span");marker.className="on-hold-indicator";marker.textContent="ON HOLD";card.prepend(marker);}}});},[sortedJobs]);
  ...
  const monthGridStart=monthCursor;
  const monthWeeks=4;
  ...
  const exportData=()=>{const payload={version:3,savedAt:new Date().toISOString(),viewMode,sortMode,operators,jobs};const json=JSON.stringify(payload,null,2);localStorage.setItem("cwem-production-planner-manual-backup",json);const blob=new Blob([json],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`CWEM-production-plan-${dateKey(weekStart)}.json`;a.click();URL.revokeObjectURL(a.href);flash("Current plan saved locally");};
  const restoreManualBackup=()=>{try{const saved=localStorage.getItem("cwem-production-planner-manual-backup");if(!saved){flash("No manual backup saved");return;}const data=JSON.parse(saved);if(!Array.isArray(data.jobs)||!Array.isArray(data.operators))throw new Error();setJobs(data.jobs.map(normalizeJob));setOperators(data.operators);if(data.viewMode)setViewMode(data.viewMode);if(data.sortMode)setSortMode(data.sortMode);flash("Manual backup restored");}catch{flash("Manual backup could not be restored");}};
  ...
  const importJobs=async(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];if(!file)return;try{let incoming:Partial<Job>[];const name=file.name.toLowerCase();if(name.endsWith(".xlsx")||name.endsWith(".xls")){const workbook=XLSX.read(await file.arrayBuffer(),{type:"array"}),sheet=workbook.Sheets[workbook.SheetNames[0]];incoming=XLSX.utils.sheet_to_json<Partial<Job>>(sheet,{defval:""});}else{const text=await file.text();if(name.endsWith(".json")){const data=JSON.parse(text);incoming=Array.isArray(data)?data:data.jobs;}else{const rows=parseCsv(text),headers=rows.shift()?.map(h=>h.toLowerCase())||[];incoming=rows.map(row=>Object.fromEntries(headers.map((header,i)=>[header,row[i]||""])));}}if(!Array.isArray(incoming)||!incoming.length)throw new Error();const batch=Date.now();const added=incoming.map((job,index)=>{const normalized=normalizeJob({...job,title:String(job.title||job.name||"Imported job"),duration:Number(job.duration)||Number(job.hours)*60||60,priority:Number(job.priority)||0,operatorId:null});return{...normalized,id:`job-${batch}-${index}`,operatorId:null};});setJobs(old=>[...old,...added]);flash(`${added.length} jobs added to list`);}catch{flash("Bulk upload could not be read");}e.target.value="";};
  const scheduleBacklog=(job:Job,operatorId:string)=>{if(!operatorId)return;if(job.onHold){flash("On hold jobs cannot be scheduled");return;}setJobs(old=>shiftCollidingJobs(old.map(j=>j.id===job.id?{...j,operatorId,lane:0}:j),job.id));};
  ...
  const undoLastBulkUpload=()=>{const timestamps=jobs.map(job=>{const match=job.id.match(/^job-(\d+)-\d+$/);return match?Number(match[1]):0;});const latest=Math.max(...timestamps);if(!latest){flash("No bulk upload found");return;}setConfirmation({message:"Remove the most recent bulk-uploaded jobs from the list?",action:()=>{setJobs(old=>old.filter(job=>!job.id.startsWith(`job-${latest}-`)));flash("Last bulk upload removed");}});};
  ...
  return <main className="app-shell" style={displayHeight?{"--planner-display-height":`${displayHeight}px`} as React.CSSProperties:undefined}>
    <header className="topbar"><div className="brand-mark"><img src={logoUrl} alt="CWEM" /></div><div className="brand-copy"><span>CWEM OPERATIONS</span><h1>Production Planner</h1></div><div className="header-actions"><button className="button ghost" onClick={()=>window.print()}>▣ Print 4 Weeks / PDF</button><button className="button ghost" onClick={exportData}>↓ Save plan</button><button className="button ghost" onClick={()=>fileRef.current?.click()}>↑ Restore</button><button className="button ghost" onClick={restoreManualBackup}>↶ Roll back</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={importData}/><button className="button ghost" onClick={addOperator}>＋ Operator</button><button className="button ghost" onClick={()=>bulkFileRef.current?.click()}>↑ Bulk upload</button><button className="button ghost" onClick={undoLastBulkUpload}>↶ Undo upload</button><input ref={bulkFileRef} type="file" accept=".xlsx,.xls,.csv,.json,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv,application/json" hidden onChange={importJobs}/><button className="button ghost" onClick={addUnavailability}>＋ Unavailability</button><button className="button primary" onClick={()=>openEditor(blankJob(currentWeekKey))}>＋ Add job to list</button></div></header>
    ...
    <section className="planner-card"><div className="planner-toolbar"><div className="week-nav"><button aria-label={viewMode==="day"?"Previous working day":viewMode==="week"?"Previous week":"Previous 4 weeks"} onClick={()=>viewMode==="day"?setDayCursor(shiftWorkingDay(dayCursor,-1)):viewMode==="week"?setWeekStart(addDays(weekStart,-7)):shiftMonth(-1)}>‹</button><button className="today" onClick={()=>{const now=new Date();if(viewMode==="day")setDayCursor(workingDay(now));else if(viewMode==="week")setWeekStart(mondayOf(now));else setMonthCursor(mondayOf(now));}}>Today</button><button aria-label={viewMode==="day"?"Next working day":viewMode==="week"?"Next week":"Next 4 weeks"} onClick={()=>viewMode==="day"?setDayCursor(shiftWorkingDay(dayCursor,1)):viewMode==="week"?setWeekStart(addDays(weekStart,7)):shiftMonth(1)}>›</button><h2>{viewMode==="day"?dayLabel:viewMode==="week"?weekLabel:monthLabel}</h2></div>{viewMode==="week"&&<div className="four-week-nav" aria-label="Four week planning horizon">{Array.from({length:4},(_,i)=>{const d=addDays(mondayOf(new Date()),i*7);return <button className={dateKey(d)===currentWeekKey?"active":""} key={i} onClick={()=>setWeekStart(d)}>W{i+1}<span>{prettyDate(d)}</span></button>;})}</div>}<div className="toolbar-colour-key">{categories.map(c=><span key={c.id} title={c.label}><i style={{backgroundColor:palette[c.id]}} />{c.label} ({jobs.filter(job=>job.category===c.id).length})</span>)}</div><div className="toolbar-right"><div className="view-switch" aria-label="Planner view"><button className={viewMode==="day"?"active":""} onClick={()=>{setDayCursor(workingDay(weekStart));setViewMode("day");}}>Day</button><button className={viewMode==="week"?"active":""} onClick={()=>setViewMode("week")}>Week</button><button className={viewMode==="month"?"active":""} onClick={()=>{setMonthCursor(mondayOf(new Date()));setViewMode("month");}}>Month</button></div><span className="autosave"><i/> Saved on this device</span><button className={`backlog-toggle ${showBacklog?"active":""}`} onClick={()=>setShowBacklog(!showBacklog)}>Job list ({jobs.length})</button></div></div>
    ...
    {editor&&<div className="modal-backdrop" onMouseDown={()=>setEditor(null)}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="job-editor" onMouseDown={e=>e.stopPropagation()}><header><div><span>{editor.id?"EDIT SCHEDULED WORK":"NEW PRODUCTION JOB"}</span><h2 id="job-editor">{editor.id?editor.title:"Add a job"}</h2></div><button onClick={()=>setEditor(null)}>×</button></header><div className="form-grid">
      ...
      <label>Quantity<input value={editor.quantity} onChange={e=>setEditor({...editor,quantity:e.target.value})}/></label><label className="hold-field"><input type="checkbox" checked={editor.onHold} onChange={e=>setEditor({...editor,onHold:e.target.checked,operatorId:e.target.checked?null:editor.operatorId})}/> ON HOLD</label>{editor.onHold&&<label className="wide">On hold reason<textarea value={editor.onHoldReason} onChange={e=>setEditor({...editor,onHoldReason:e.target.value})} placeholder="Why is this job on hold?"/></label>}<label>Due date<input type="date" value={editor.due} onChange={e=>setEditor({...editor,due:e.target.value})}/></label>
