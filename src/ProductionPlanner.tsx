...
export default function ProductionPlanner(){
  ...
  const shiftMonth=(amount:number)=>{const target=new Date(monthDate.getFullYear(),monthDate.getMonth()+amount,1);setWeekStart(mondayOf(target));};
  ...
  return <main className="app-shell">
    ...
    <section className="planner-card"><div className="planner-toolbar"><div className="week-nav"><button aria-label={viewMode==="week"?"Previous week":"Previous month"} onClick={()=>viewMode==="week"?setWeekStart(addDays(weekStart,-7)):shiftMonth(-1)}>‹</button><button className="today" onClick={()=>setWeekStart(mondayOf(new Date()))}>Today</button><button aria-label={viewMode==="week"?"Next week":"Next month"} onClick={()=>viewMode==="week"?setWeekStart(addDays(weekStart,7)):shiftMonth(1)}>›</button><h2>{viewMode==="week"?weekLabel:monthLabel}</h2></div>{viewMode==="week"&&<div className="four-week-nav" aria-label="Four week planning horizon">{Array.from({length:4},(_,i)=>{const d=addDays(mondayOf(new Date()),i*7);return <button className={dateKey(d)===currentWeekKey?"active":""} key={i} onClick={()=>setWeekStart(d)}>W{i+1}<span>{prettyDate(d)}</span></button>;})}</div>}<div className="toolbar-right"><div className="view-switch" aria-label="Planner view"><button className={viewMode==="week"?"active":""} onClick={()=>setViewMode("week")}>Week</button><button className={viewMode==="month"?"active":""} onClick={()=>setViewMode("month")}>Month</button></div><span className="autosave"><i/> Saved on this device</span>{viewMode==="week"&&<button className={`backlog-toggle ${showBacklog?"active":""}`} onClick={()=>setShowBacklog(!showBacklog)}>Unscheduled ({backlog.length})</button>}</div></div>
